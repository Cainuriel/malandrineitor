  ...
  function render(c) {
    ...
    c.appendChild(el('div', { class: 'rules' }, [
      ...
      sec('Modos de juego', [
        ...
        '<b>Historia</b>: fichas por Malandriner S.A. con un sobre de bienvenida y ' + cfg.story.startCoins + ' malandricoins. Juegas ' + cfg.story.chapters.length + ' capítulos contra ' + cfg.rival.name + ' con una plantilla elegida de tu colección, cobras por ticket resuelto y por sprint ganado, compras sobres (básico, pro y calabozo) y descubres el álbum. Hay puntos de control: si pierdes un sprint retrocedes al último alcanzado, nunca pierdes cartas ni malandricoins. Las cartas repetidas se venden en la colección según su rareza (' + Object.entries(cfg.story.sellPrice).map(([r, v]) => cfg.rarities[r].name.toLowerCase() + ' ' + v).join(', ') + '). El último capítulo es la auditoría.',
